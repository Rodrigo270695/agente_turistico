...
watch(selectedDepartment, (newValue) => {
    if (newValue) {
        const selectedDepartment = props.departments.find(department => department.name === newValue);
...
watch(selectedProvince, (newValue) => {
    if (newValue) {
        const department = props.departments.find(department => department.name === selectedDepartment.value);
        if (department) {
            const province = department.provinces.find(province => province.name === newValue);
            if (province) {
...
<form @submit.prevent="submit">
        <div class="bg-white shadow-md rounded-md p-4">
            <div class="text-lg font-bold mb-4">{{ form.id == 0 ? 'Registrar Lugar': 'Actualizar Lugar'}}</div>
            <div class="mb-4">
                ...
                <div class="grid grid-cols-6 gap-3">
                    ...
                    <div class="col-span-6 sm:col-span-3">
                        ...
                        <select id="select" v-model="selectedDepartment"
                        ...
                        <InputError class="w-full" :message="form.errors.department" />
                    </div>

                    <TextInput type="hidden" class="w-full" v-model="form.department" :value="selectedDepartment" />

                    <div class="col-span-6 sm:col-span-3">
                        <InputLabel value="Provincia" />
                        <select id="select" v-model="selectedProvince"
                            class="bg-gray-50 border border-blue-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 white:bg-gray-700 white:border-gray-600 white:placeholder-gray-400 white:text-white white:focus:ring-blue-500 white:focus:border-blue-500">
                            <option disabled selected value="">
                                Seleccione un opción
                            </option>
                    ...
                    <TextInput type="hidden" class="w-full" v-model="form.province" :value="selectedProvince" />

                    <div class="col-span-6 sm:col-span-3">
                        <InputLabel value="Distrito" />
                        <select id="select" v-model="form.district"
                            class="bg-gray-50 border border-blue-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 white:bg-gray-700 white:border-gray-600 white:placeholder-gray-400 white:text-white white:focus:ring-blue-500 white:focus:border-blue-500">
                            <option disabled selected value="">
                                Seleccione un opción
                            </option>
                        ...
                        <InputError class="w-full" :message="form.errors.district" />
                    </div>

                    <div class="col-span-6 sm:col-span-6">
                        <InputLabel value="Dirección" />
                        <TextArea class="w-full" v-model="form.address" />
                        <InputError class="w-full" :message="form.errors.address"/>
                    </div>

                    <div class="col-span-6 sm:col-span-3">
                        <InputLabel value="Tipo de lugar"  />
                        <select id="select"  v-model="form.type_place" class="bg-gray-50 border border-blue-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 white:bg-gray-700 white:border-gray-600 white:placeholder-gray-400 white:text-white white:focus:ring-blue-500 white:focus:border-blue-500">
                            <option disabled selected value="">
                                Seleccione un opción
                            </option>
                            <option v-for="dato in datos" :key="dato.id" :value="dato.name">{{ dato.name }}</option>
                        </select>
                        <InputError class="w-full" :message="form.errors.type_place"/>
                    </div>

                    <div class="col-span-6 sm:col-span-3">
                        <InputLabel value="Días de atención" />
                        <TextInput class="w-full" v-model="form.care_day"  />
                        <InputError class="w-full" :message="form.errors.care_day"/>
                    </div>

                    <div class="col-span-6 sm:col-span-3">
                        <InputLabel value="Horas de atención" />
                        <TextInput class="w-full" v-model="form.opening_hours"  />
                        <InputError class="w-full" :message="form.errors.opening_hours"/>
                    </div>

                    <div class="col-span-6 sm:col-span-3">
                        <InputLabel value="Precio" />
                        <TextInput type="number" class="w-full" v-model="form.price"  />
                        <InputError class="w-full" :message="form.errors.price"/>
                    </div>


                    <div class="col-span-6 sm:col-span-3">
                        <InputLabel value="Acceso"  />
                        <select id="select"  v-model="form.access" class="bg-gray-50 border border-blue-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 white:bg-gray-700 white:border-gray-600 white:placeholder-gray-400 white:text-white white:focus:ring-blue-500 white:focus:border-blue-500">
                            <option disabled selected value="">
                                Seleccione un opción
                            </option>
                            <option v-for="dato in access" :key="dato.id" :value="dato.name">{{ dato.name }}</option>
                        </select>
                        <InputError class="w-full" :message="form.errors.access"/>
                    </div>
